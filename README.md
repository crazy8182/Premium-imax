...
## Koyeb polling + health check

This build runs Telegram polling and an HTTP health endpoint on `0.0.0.0:$PORT`.
It also removes an old Telegram webhook before starting polling. Koyeb Web Service should expose TCP port 8080.
Startup logs show the bot username and every received update.
