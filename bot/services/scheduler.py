...
async def process(bot):
    ...
    async for user in expired_users():
        ...
        await upsert_user(uid, premium_status=False, joined_group=False, expired_offer_until=offer_until, expired_offer_reminders_sent=1, expired_offer_last_reminder=now)
        plan_name = user.get('premium_plan_name') or 'Premium'
        try:
            from bot.keyboards import expired_offer_menu
            await bot.send_message(uid, bold_small_caps(f'🔴 Your {plan_name} premium membership has expired.\n\n🎁 Special Offer: {EXPIRED_DISCOUNT_PERCENT}% OFF\n⏳ Valid only for the next {EXPIRED_OFFER_DAYS} days.\n\nRenew now and get your premium at a special price.\nAfter these 3 days, normal plan prices will apply.'), reply_markup=expired_offer_menu(), parse_mode='HTML')
        except Exception as e:
    ...
    async for user in active_users():
        ...
        try:
            ...
            sent = await bot.send_message(uid, bold_small_caps('⏰ Reminder: your Movie premium is active, but you have not joined the Premium Group yet.\n\nUse the button below to join the Premium Group.'), reply_markup=join_menue(), parse_mode='HTML')
    ...
    async for user in __import__('bot.db', fromlist=['users']).users.find({'premium_status': False, 'expired_offer_until': {'$gt': now}}):
        ...
        try:
            from bot.keyboards import expired_offer_menu
            await bot.send_message(uid, bold_small_caps(f"🔥 Don't miss your expired-user offer!\n\n🎁 {EXPIRED_DISCOUNT_PERCENT}% OFF Premium\n⏳ About {remaining_days} day(s) / {remaining_hours} hour(s) remaining.\n\nAfter the offer expires, normal plan prices will apply."), reply_markup=expired_offer_menu(), parse_mode='HTML')
            await upsert_user(uid, expired_offer_last_reminder=now, expired_offer_reminders_sent=sent + 1)
        except Exception as e:
